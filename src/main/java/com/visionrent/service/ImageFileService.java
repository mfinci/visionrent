...
public class ImageFileService {
	...
	public ImageFile getImageById(String id) {
		ImageFile imageFile = imageFileRepository.findById(id).orElseThrow(
				() -> new ResourceNotFoundException(String.format(ErrorMessage.IMAGE_NOTFOUND_MESSAGE, id)));
	...
	public List<ImageFileDTO> getAllImages() {
		...
		List<ImageFileDTO> imageFileDTOs = imageFiles.stream().map(imFile -> {
			...
			String imageUri = ServletUriComponentsBuilder.// URI build eden hazır method
			fromCurrentContextPath().// lochalhost:8080
			path("/files/dowload/").
			path(imFile.getId()).
			toString();// end-point sonuna image id eklenmiş
...
//**************************************************************
	public void removeByIde(String id) {
